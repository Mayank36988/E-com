...
//     const token = Cookies.get("access_token"); // Correct key for access token
...
//     if (token) {
//         fetchUserProfile(token); // Pass token to the fetch function
//     } else {
//         console.warn("Access Token is not found in cookies");
//     }
// }, []);

// const fetchUserProfile = async (token) => { // Accept token as argument
//     try {
...
//           console.log('Profile Data:', data); // Is line ko check karein
//           setUserProfile(data);
//       }
...
//         Cookies.remove("access_token"); // Logout ke baad token ko bhi clear karo
//         onLogout();
//         navigate("/login");
//       }
//     } catch (error) {
...
//                         <p className="text-xs">Address: {userProfile.address}</p>
//                       </div>
//                     )}
